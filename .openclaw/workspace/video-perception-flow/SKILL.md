...
**2.1 读取会议数据**
每次触发本 Skill 时，均重新读取 `~/.openclaw/workspace/SimulatedData/meetings.json`，解析为 JSON 数组；不使用跨次触发缓存。

**2.2 按 zone + timestamp 匹配**

- 按 `zone` 精确匹配
- 从 `time_range`（如 `"2026-06-15 14:00~15:30"`）解析日期和起止时间
- 判定 event timestamp 是否落在范围内
...
**4.1 读取客户画像**
在 `~/.openclaw/workspace/SimulatedData/profiles/` 下按 `customer_company` 模糊匹配对应的 Markdown 文件。

- 文件存在 → 解析每位访客的职位(title)、历史来访次数(visit_count)、特殊接待需求(special)
- 文件不存在 → 记录警告，跳过个性化信息，仅用会议数据生成通用欢迎语（状态标记 partial）
...
在推送欢迎卡片的同时，触发总体控制流程（overall-control）的自动调节模式。
...
- 环境调节和设备控制反馈由 overall-control 独立完成，本流程不等待其执行结果
...
```json
{
  ...
  "skill_name": "video-perception-flow",
  "version": "1.1.0",
...
| 文件          | 路径                                                                |
| ------------- | ------------------------------------------------------------------- |
| 区域-设备映射 | `skills/video-perception-flow/references/zone-device-mapping.md`    |
| 会议预订数据  | `~/.openclaw/workspace/SimulatedData/meetings.json`                 |
| 欢迎模板      | `~/.openclaw/workspace/SimulatedData/templates/welcome-template.md` |
| 客户画像      | `~/.openclaw/workspace/SimulatedData/profiles/`（多个 .md 文件）    |
