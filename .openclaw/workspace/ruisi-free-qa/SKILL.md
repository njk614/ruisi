...
15. 可选留痕到 `trace-workspace/ruisi-free-qa/<date>/<session_id>.json`，建议记录 `meeting_match`、匹配到的时间/主题（若有）、归一化意图与详细度级别。
...
| `scripts/pause_explanation.py`   | 调用 ruisi-explanation-service 的消息入口，触发暂停讲解     |
...
ruisi-free-qa 默认通过 `scripts/pause_explanation.py` 调用 ruisi-explanation-service 的消息入口脚本，并传入上面的暂停消息。
...
- ruisi-explanation-service 消息入口、真实 P02 发送 API 和 OpenClaw 大模型服务调用需要相应权限。
